...
public class Poblacio {
    ...
    public int quantes(char inici, char fi, int tipusSenyal){
        ...
        for (int x=0; x<3;x++){
            ...
            for (int y=0; y<col;y++){
                SenyalTransit s= ubicacio[x][y].getSenyal(y);
    ...
    private int recurregutMetodeQuantes (int tipus, int length, int fila){
        ...
        for (int y=0; y<numIN;y++){
            SenyalTransit s= ubicacio[fila][y].getSenyal(y);
            if (getTipusSenyal(s).equals(tipusSenyal(tipus))){
                total=total+ubicacio[fila][y].getNumSenyals();
            }
        }
        return total;
    ...
    private String getTipusSenyal(SenyalTransit s){
        if ((s instanceof Advertencia)) {
            return "Advertenica";
        } else if ((s instanceof Reglamentacio)) {
            return "Reglamentacio";
        } else if ((s instanceof Indicacio)) {
            return "Indicacio";
        }
        return null;
    }
